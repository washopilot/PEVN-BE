...
authentication.signUp = async (req, res) => {
  ...
  if (role == 'professor') {
    try {
      await pool.query('INSERT INTO professor (p_name, p_email, p_password) VALUES ($1, $2, $3)', [
        name,
        email,
        password,
      ]);
    ...
    try {
      await pool.query('INSERT INTO student (s_name, s_email, s_password) VALUES ($1, $2, $3)', [
        name,
        email,
        password,
      ]);
  ...
  if (role == 'professor') {
    try {
      const professor = await (
        await pool.query('SELECT * FROM professor WHERE p_email=$1 AND p_password=$2', [email, password])
      ).rows;
    ...
    try {
      const student = await (
        await pool.query('SELECT * FROM student WHERE s_email=$1 AND s_password=$2', [email, password])
      ).rows;
      if (student.length > 0) {
        res.status(200).json({
          id: student[0].id_s,
